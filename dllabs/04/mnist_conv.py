...
class Network:
    ...
    def construct(self, args):
        with self.session.graph.as_default():
            ...
            for disc in args.cnn.split(","):
                arr = disc.split("-")
                t, pa = arr[0].lower(), arr[1:]
                # - C-filters-kernel_size-stride-padding: Add a convolutional layer with ReLU activation and
                #   specified number of filters, kernel size, stride and padding. Example: C-10-3-1-same
                if t == "c":
                ...
                if t == "m":
                    features = tf.layers.max_pooling2d(
                        features,
                        pool_size=[int(pa[0]), int(pa[0])],
                        strides=int(pa[1]),
                        name=disc
                    )
                # - F: Flatten inputs
                if t == "f":
                    features = tf.layers.flatten(features, name=disc)
                # - R-hidden_layer_size: Add a dense layer with ReLU activation and specified size. Ex: R-100
                if t == "r":
                    features = tf.layers.dense(
                        features, int(pa[0]), activation=tf.nn.relu, name=disc
                    )


            # just experiment
            #features = tf.layers.dense(self.images, 50, activation=tf.nn.relu)
            #features = tf.layers.dropout(features, training=self.is_training, name="dropout")
...
if __name__ == "__main__":
    ...
    for i in range(args.epochs):
        ...
        #print("Epoch {} dev: {:.2f}".format(i + 1, 100 * accuracy))

...
class Network:
    ...
    def construct(self, args, source_chars, target_chars, bow, eow):
        ...
        with self.session.graph.as_default():
            ...
            source_encoded = outputs[0] + outputs[1] # shape: (word, char, vals)
            source_states = states[0] + states[1] # shape: (word, vals)


            # Index the unique words using self.source_ids and self.target_ids.


            sentence_mask = tf.sequence_mask(self.sentence_lens) # shape: (sen, word)

            source_encoded = tf.boolean_mask(tf.nn.embedding_lookup(source_encoded, self.source_ids), sentence_mask) # shape: (word, char, vals)
            source_states = tf.boolean_mask(tf.nn.embedding_lookup(source_states, self.source_ids), sentence_mask) # shape: (word, vals)
            source_lens = tf.boolean_mask(tf.nn.embedding_lookup(self.source_seq_lens, self.source_ids), sentence_mask) # shape: (word)

            target_seqs = tf.boolean_mask(tf.nn.embedding_lookup(target_seqs, self.target_ids), sentence_mask) # shape: (word, charid)
            target_lens = tf.boolean_mask(tf.nn.embedding_lookup(target_seq_lens, self.target_ids), sentence_mask) #shape: (word)

            #print(f"source_encoded: {source_encoded.shape}")
            #print(f"source_states: {source_states.shape}")
            #print(f"source_lens: {source_lens.shape}")
            #print(f"target_seqs: {target_seqs.shape}")
            #print(f"target_lens: {target_lens.shape}")
            ...
            decoder_gru = tf.nn.rnn_cell.GRUCell(args.rnn_dim)
            ...
            state_layer.build(args.rnn_dim)
            ...
            def with_attention(inputs, states):
                ...
                new_source_encoded = source_layer(source_encoded) # shape: (word, char, rnn_dim) ok

                # TODO: Change shape of states from [a, b] to [a, 1, b] and project it using state_layer.
                states = tf.reshape(states, [tf.shape(states)[0], 1, tf.shape(states)[1]])
                states = state_layer(states) # shape: (a, 1, rnn_dim) ok
                #print(f"states: {states.shape}")

                # TODO: Sum the two above projections, apply tf.tanh and project the result using weight_layer.
                # The result has shape [x, y, 1].
                projections = weight_layer(tf.tanh(new_source_encoded + states)) # shape: (a, 1, 1)
                ...
                weights = tf.nn.softmax(projections, axis=1)
                ...
                attn = tf.reduce_sum(source_encoded * weights, axis=1)
                #print(f"attn: {attn.shape}")

                # TODO: Return concatenation of inputs and the computed attention.
                inputs = tf.cast(inputs, tf.float32)
                ret = tf.concat((inputs, attn), axis=1)
                #print(f"ret (with_attention): {ret.shape}")
                return ret
            ...
            class DecoderTraining(tf.contrib.seq2seq.Decoder):
                ...
                def step(self, time, inputs, states, name=None):
                    ...
                    next_input = target_seqs_embedded[:, time, :]
                    next_input = with_attention(next_input, states)  # TODO: Next input is with_attention called on words with index `time` in target_embedded.
            ...
            class DecoderPrediction(tf.contrib.seq2seq.Decoder):
                ...
                def step(self, time, inputs, states, name=None):
                    ...
                    next_input = with_attention(next_input, states) # TODO: Embed `outputs` using target_embeddings and pass it to with_attention.
